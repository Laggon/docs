...
Распределенная система контроля версий. Так как система распределенная, она позволяет хранить репозиторий [локально](#local-repository---локальный-репозиторий-репозиторий) у каждого разработчика и в [удаленном репозитории](#remote-repository---удаленный-репозиторий).
...
В рамках [PR](#pull-request-pr---запрос-на-принятие-изменений) можно запросить ревью у одного или нескольких участников.
...
Требуемые изменения вносятся в той же ветке [PR](#pull-request-pr---запрос-на-принятие-изменений), после внесения всех необходимых изменений снова запрашивается ревью.
...
## nit - Необязательные правки

Данным префиксом обозначаются правки, которые не обязательно вносить. Например:

```text
nit: Данный цикл можно написать например с помощью метода abc
```
